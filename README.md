...
下一步：狼人要知道他的队友都是谁，发言有点小bug，平票发言还没写
...
发言时，先发送发言阶段开始包，发给房内所有玩家，客户端显示阶段情况；

后给发言人发送开始发言包，发言人开始发言，发言结束后，服务端给下一个人发送开始发言包。
...
下一步：完善发言时提示下一位发言人，警长选择发言顺序，退出房间要清楚该房间的数据，同时登录将上一个登录ip的sockfd删除
...
解决强制退出上一个登录时开始游戏后，新用户会收到两次信息包的bug：在服务端删除原登录客户端的sockfd时，参数传错了，房间成员列表保存的是用户的id，删除使用用户座位号删除了。。。

完善发言

完善退出房间清除数据：在加入新房间时会调用setinfo函数进行房间信息设置，在该函数内部进行房间信息的初始化。
...
白天放逐投票统计投票结果，票数最多的人出局，出局后，可以进行淘汰发言；

如果票数最高的有多个人，无人被放逐，直接进入夜晚

玩家淘汰或退出，要在服务端从存活列表中删除（用于控制发言的mymap）

客户端计算出放逐玩家后，显示被放逐的玩家，然后发送给服务端；服务端收到放逐通知后，更新房间的各职位信息，判断游戏有没有结束；没结束的话，将放逐玩家发言的包发送给所有玩家，放逐玩家开始发言（服务端发完发言包后，将淘汰玩家从发言列表中删除）；发言结束后，服务端给所有玩家发送淘汰包，客户端收到淘汰包后，更新淘汰玩家的状态信息

玩家退出的时候，如果是出局状态，不用更新服务端的房间信息
...
完成游戏基本流程（有bug），添加语音发送接收播放



到某人发言的时候，服务端发送给每个人一个发言包，其内包含发言人的座位号，客户端显示：xx号正在发言；

在发言人发言时，将语音数据包发送给房间内每个人，房内人将方言人的头像高光显示；

在发言人暂停发言时，房内人将发言人的头像换个颜色；

在发言人结束发言时，房内人将发言人的头像恢复。

以上三个事件发言人都要向服务器发送包，服务器将包转发给房内每个人。
...
1. ~~（完善）夜间狼人可以和队友沟通~~
2. ~~操作提示不明显~~
3. ~~上警后的标志（上、放、警）等没有进行更新~~
4. ~~警长选择发言顺序：如果超时没有选择，默认顺序~~
5. ~~发言提示：请xx做好准备（玩家号码不准确）~~
6. ~~发言有时间限制，（120s）到时间自动发送结束包~~
7. ~~开始发言后高亮显示，结束发言后恢复正常~~
8. 警长死亡后要选择是否移交警徽
9. 狼人有自爆键，自爆后进行淘汰发言，发言后进入天黑
10. 死亡玩家只能接收，不能发送（除了房主），不能进行操作
11. 游戏结束后，显示胜利玩家，和玩家身份信息
12. 游戏结束后，还在房内的玩家保留上一轮的数据，在房主点击开始游戏后进行清屏和房内信息初始化
13. 警上发言不显示玩家号码
14. 信息框美化
15. 身份显示：好人阵营的玩家本人显示具体身份信息
16. 游戏开始后，退出游戏框叉掉后依然会退出
17. 游戏开始，是否退出游戏和是否退出游戏只留一个
18. （完善）观战功能
19. 密码加密显示
20. 发言阶段进行提示
21. 女巫用完药之后的夜间显示
